      ...
      <ul v-else class="divide-y divide-gray-100">
        ...
        >
          <div class="flex items-start gap-3">
            <!-- Icon -->
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-orange-100 text-orange-700 mt-0.5">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-start justify-between gap-2">
                <div class="flex flex-wrap items-center gap-1.5 min-w-0">
                  ...
                  >{{ $t('orders.requires_delivery') }}</span>
                </div>
              ...
              <div class="mt-1 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">
                  {{ [item.courier?.name, item.user?.name, formatDate(item.created_at)].filter(Boolean).join(' · ') }}
                </p>
...
async function fetchItems(url = null) {
  ...
  const { data } = await get(url ?? '/api/v1/orders', url ? {} : params)
...
async function doDelete() {
  ...
  await del(`/api/v1/orders/${deleteTarget.value.id}`)
  await fetchItems()
}
