<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <!-- Totals footer -->
      <div v-if="form.items.length > 0" class="flex items-end justify-between border-t border-gray-200 px-5 py-4 bg-gray-50 rounded-b-xl">
...
async function fetchOptions() {
  const [clientsRes, productsRes] = await Promise.all([get('/api/clients'), get('/api/products?per_page=500')])
  ...
  for (const product of products) {
    for (const pp of (product.presentations ?? [])) {
      ...
      flat.push({ id: pp.id, productName: product.name, presentationDisplay: pp.presentation?.display ?? '', price: pp.price ?? 0, stock: pp.stock ?? 0 })
    }
  }
