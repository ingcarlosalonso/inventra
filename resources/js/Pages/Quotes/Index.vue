  ...
  <div class="space-y-6">
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition cursor-pointer" @click="() => router.visit(`/quotes/${item.id}`)">
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                <p class="truncate text-sm font-medium text-gray-900">{{ item.client?.name ?? $t('quotes.no_client') }}</p>
                <span class="inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset" :class="item.is_converted ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-amber-50 text-amber-700 ring-amber-200'">{{ item.is_converted ? $t('quotes.converted') : $t('quotes.not_converted') }}</span>
              </div>
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ [item.expires_at ? `${$t('quotes.expires_at')}: ${formatDate(item.expires_at)}` : null, item.user?.name, formatDate(item.created_at)].filter(Boolean).join(' · ') }}</p>
                <div class="flex items-center gap-1 shrink-0" @click.stop>
                  <span class="text-sm font-semibold text-gray-900 tabular-nums mr-1">${{ formatNumber(item.total) }}</span>
                  <Link :href="`/quotes/${item.id}`" class="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-indigo-600 sm:opacity-0 sm:group-hover:opacity-100 transition" :title="$t('common.view')"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></Link>
                  <button v-if="!item.is_converted" type="button" class="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-red-600 sm:opacity-0 sm:group-hover:opacity-100 transition" :title="$t('common.delete')" @click="confirmDelete(item)"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg></button>
                </div>
              </div>
            </div>
          </div>
...
async function fetchItems(url = null) {
  ...
  const { data } = await get(url ?? '/api/v1/quotes', url ? {} : params)
