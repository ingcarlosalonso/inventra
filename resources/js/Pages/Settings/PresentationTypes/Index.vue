<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('presentation_types.title') }}</h1>
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
          ...
          <span class="rounded bg-gray-100 px-2 py-0.5 text-xs font-mono text-gray-600 shrink-0">{{ item.abbreviation }}</span>
  ...
  <!-- Slide-over form -->
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('presentation_types.edit') : $t('presentation_types.create')">
    <form class="space-y-5" @submit.prevent="save">
      ...
      <InputField
        ...
        :label="$t('presentation_types.abbreviation')"
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('presentation_types.delete_confirm', { name: deleteTarget?.name })"
    @confirm="doDelete"
  />
</template>
...
const form = ref({ name: '', abbreviation: '', is_active: true })
...
function openCreate() {
  editing.value = null
  form.value = { name: '', abbreviation: '', is_active: true }
...
function openEdit(item) {
  editing.value = item
  form.value = { name: item.name, abbreviation: item.abbreviation, is_active: item.is_active }
  formError.value = null
  slideOverOpen.value = true
}
...
async function save() {
  formError.value = null
  const payload = { name: form.value.name, abbreviation: form.value.abbreviation, is_active: form.value.is_active }
