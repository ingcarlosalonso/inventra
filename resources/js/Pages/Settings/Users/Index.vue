<template>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ item.email }}</p>
                <div class="flex items-center gap-1.5 shrink-0">
                  <div class="hidden sm:flex items-center gap-1">
                    <span v-for="role in item.roles.slice(0, 2)" :key="role.id" class="inline-flex items-center rounded-full bg-violet-100 px-2 py-0.5 text-xs font-medium text-violet-700">{{ role.name }}</span>
                    <span v-if="item.roles.length > 2" class="text-xs text-gray-400">+{{ item.roles.length - 2 }}</span>
                  </div>
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('users.edit') : $t('users.create')">
    <form class="space-y-5" @submit.prevent="save">
      <InputField v-model="form.name" :label="$t('users.name')" :error="formErrors.name?.[0]" required />
      <InputField v-model="form.email" :label="$t('users.email')" type="email" :error="formErrors.email?.[0]" required />
      <div>
        <InputField v-model="form.password" :label="$t('users.password')" type="password" :error="formErrors.password?.[0]" :required="!editing" />
        <p v-if="editing" class="mt-1 text-xs text-gray-400">{{ $t('users.password_hint') }}</p>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ $t('users.roles') }}</label>
        <div class="flex flex-wrap gap-2">
          <label
            v-for="role in availableRoles"
            :key="role.id"
            class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition"
            :class="form.roles.includes(role.id) ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-700 hover:border-gray-300'"
          >
            <input type="checkbox" class="sr-only" :value="role.id" v-model="form.roles" />
            {{ role.name }}
          </label>
        </div>
        <p v-if="formErrors.roles?.[0]" class="mt-1 text-xs text-red-600">{{ formErrors.roles[0] }}</p>
      </div>
  ...
  <ConfirmModal v-model="confirmOpen" :title="$t('users.delete_confirm', { name: deleteTarget?.name })" @confirm="doDelete" />
</template>
...
const emptyForm = () => ({ name: '', email: '', password: '', roles: [], is_active: true })
...
async function save() {
  ...
  const result = editing.value
    ? await putForm(`/api/v1/settings/users/${editing.value.id}`, payload)
...
async function doDelete() { confirmOpen.value = false; await del(`/api/v1/settings/users/${deleteTarget.value.id}`); await fetchItems() }
