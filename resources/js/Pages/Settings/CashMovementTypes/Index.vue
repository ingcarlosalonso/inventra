<template>
  <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-gray-900">{{ $t('cash_movement_types.title') }}</h1>
      ...
      <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
        ...
        <ul v-else class="divide-y divide-gray-100">
          <li v-for="item in items" :key="item.id" class="group flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
            <p class="flex-1 truncate text-sm font-medium text-gray-900">{{ item.name }}</p>
            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 shrink-0" :class="item.is_income ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-rose-50 text-rose-700 ring-rose-200'">{{ item.is_income ? $t('cash_movement_types.direction_in') : $t('cash_movement_types.direction_out') }}</span>
            <StatusBadge :active="item.is_active" />
            <div class="flex items-center gap-0.5 shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition">
              <button type="button" class="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-indigo-600" @click="openEdit(item)"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg></button>
    ...
    <SlideOver v-model="slideOverOpen" :title="editing ? $t('cash_movement_types.edit') : $t('cash_movement_types.create')">
      <form class="space-y-5" @submit.prevent="save">
        <InputField v-model="form.name" :label="$t('common.name')" :error="formErrors.name?.[0]" required />
        <SelectField v-model="form.is_income" :label="$t('cash_movement_types.direction')" :options="directionOptions" required />
    ...
    <ConfirmModal v-model="confirmOpen" :title="$t('cash_movement_types.delete_confirm', { name: deleteTarget?.name })" @confirm="doDelete" />
</template>
...
function openCreate() {
    editing.value = null; form.value = { name: '', is_income: true, is_active: true }; formError.value = null; slideOverOpen.value = true
}

function openEdit(item) {
    editing.value = item; form.value = { name: item.name, is_income: item.is_income, is_active: item.is_active }; formError.value = null; slideOverOpen.value = true
}
