<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('currencies.title') }}</h1>
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-sm font-bold text-amber-700">{{ item.symbol }}</div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                <p class="truncate text-sm font-medium text-gray-900">{{ item.name }}</p>
                <div class="flex items-center gap-1.5 shrink-0">
                  <span v-if="item.is_default" class="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200">{{ $t('currencies.is_default') }}</span>
                  <StatusBadge :active="item.is_active" />
                </div>
              </div>
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="text-xs text-gray-400 font-mono">{{ item.iso_code }}</p>
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('currencies.edit') : $t('currencies.create')">
    <form class="space-y-5" @submit.prevent="save">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <InputField
          ...
          :label="$t('currencies.iso_code')"
          :error="formErrors.iso_code?.[0]"
          required
        />
      </div>
      <ToggleSwitch v-model="form.is_default" :label="$t('currencies.is_default')" />
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('currencies.delete_confirm', { name: deleteTarget?.name })"
    @confirm="doDelete"
  />
</template>
...
const emptyForm = () => ({ name: '', symbol: '', iso_code: '', is_default: false, is_active: true })
...
async function save() {
    ...
    const result = editing.value
        ? await putForm(`/api/v1/settings/currencies/${editing.value.id}`, form.value)
...
async function doDelete() {
    ...
    await del(`/api/v1/settings/currencies/${deleteTarget.value.id}`)
    await fetchItems()
}
