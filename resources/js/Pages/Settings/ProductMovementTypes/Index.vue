      ...
      <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
        ...
        <ul v-else class="divide-y divide-gray-100">
          <li v-for="item in items" :key="item.id" class="group flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
            ...
            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 shrink-0" :class="item.is_income ? 'bg-blue-50 text-blue-700 ring-blue-200' : 'bg-amber-50 text-amber-700 ring-amber-200'">{{ item.is_income ? $t('product_movement_types.direction_in') : $t('product_movement_types.direction_out') }}</span>
...
async function save() {
    ...
    const result = editing.value
        ? await putForm(`/api/v1/products/movement-types/${editing.value.id}`, payload)
...
async function doDelete() {
    ...
    await del(`/api/v1/products/movement-types/${deleteTarget.value.id}`)
    await fetchItems()
}
