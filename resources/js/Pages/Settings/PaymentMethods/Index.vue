<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('payment_methods.title') }}</h1>
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <div v-if="loadingList" class="divide-y divide-gray-100">
        <div v-for="i in 5" :key="i" class="flex items-center gap-4 px-5 py-4 animate-pulse">
          <div class="h-4 w-48 rounded bg-gray-200 flex-1" />
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('payment_methods.edit') : $t('payment_methods.create')">
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('payment_methods.delete_confirm', { name: deleteTarget?.name })"
    @confirm="doDelete"
  />
</template>
...
async function fetchItems() {
    ...
    const { data } = await get('/api/v1/sales/payment-methods', params)
...
async function save() {
    ...
    const result = editing.value
        ? await putForm(`/api/v1/sales/payment-methods/${editing.value.id}`, form.value)
        : await postForm('/api/v1/sales/payment-methods', form.value)
    if (result.error) {
...
async function doDelete() {
    ...
    await del(`/api/v1/sales/payment-methods/${deleteTarget.value.id}`)
    await fetchItems()
}
