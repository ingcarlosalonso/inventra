          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ item.phone ?? item.email ?? '—' }}</p>
...
async function fetchItems() {
  ...
  const { data } = await get('/api/v1/orders/couriers', params)
...
async function save() {
  ...
  const result = editing.value
    ? await putForm(`/api/v1/orders/couriers/${editing.value.id}`, form.value)
    : await postForm('/api/v1/orders/couriers', form.value)
  if (result.error) {
...
async function doDelete() {
  ...
  await del(`/api/v1/orders/couriers/${deleteTarget.value.id}`)
  await fetchItems()
}
