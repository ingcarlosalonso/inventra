<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('sale_states.title') }}</h1>
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition">
          <div class="flex items-center gap-3">
            <div class="h-4 w-4 shrink-0 rounded-full ring-1 ring-gray-200" :style="item.color ? { backgroundColor: item.color } : { backgroundColor: '#e5e7eb' }" />
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                <p class="truncate text-sm font-medium text-gray-900">{{ item.name }}</p>
                <div class="flex items-center gap-1.5 shrink-0">
                  <span v-if="item.is_default" class="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200">{{ $t('sale_states.is_default') }}</span>
                  <span v-if="item.is_final_state" class="inline-flex items-center rounded-full bg-rose-50 px-2 py-0.5 text-xs font-medium text-rose-700 ring-1 ring-rose-200">{{ $t('sale_states.is_final_state') }}</span>
                </div>
              </div>
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ $t('sale_states.sort_order') }}: {{ item.sort_order }}</p>
                <div class="flex items-center gap-1.5 shrink-0">
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('sale_states.edit') : $t('sale_states.create')">
    <form class="space-y-5" @submit.prevent="save">
      <InputField
        v-model="form.name"
        :label="$t('sale_states.name')"
        :error="formErrors.name?.[0]"
        required
      />
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-1.5 block text-sm font-medium text-gray-700">{{ $t('sale_states.color') }}</label>
          <input
            v-model="form.color"
            type="color"
            class="h-10 w-full cursor-pointer rounded-lg border border-gray-300 p-1 shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
          />
        </div>
        <InputField
          v-model.number="form.sort_order"
          type="number"
          :label="$t('sale_states.sort_order')"
          :error="formErrors.sort_order?.[0]"
        />
      </div>
      <ToggleSwitch v-model="form.is_default" :label="$t('sale_states.is_default')" />
      <ToggleSwitch v-model="form.is_final_state" :label="$t('sale_states.is_final_state')" />
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('sale_states.delete_confirm', { name: deleteTarget?.name })"
    @confirm="doDelete"
  />
</template>
...
const emptyForm = () => ({ name: '', color: '#6366f1', is_default: false, is_final_state: false, is_active: true, sort_order: 0 })
const form = ref(emptyForm())

async function fetchItems() {
    const params = {}
    if (search.value) params.search = search.value
    const { data } = await get('/api/v1/sales/states', params)
...
async function save() {
    ...
    const result = editing.value
        ? await putForm(`/api/v1/sales/states/${editing.value.id}`, form.value)
        : await postForm('/api/v1/sales/states', form.value)
    if (result.error) {
...
async function doDelete() {
    ...
    await del(`/api/v1/sales/states/${deleteTarget.value.id}`)
    await fetchItems()
}
