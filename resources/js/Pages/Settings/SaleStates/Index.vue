  ...
  <div class="space-y-6">
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <ul v-else class="divide-y divide-gray-100">
        ...
        >
          <div
            class="h-4 w-4 shrink-0 rounded-full ring-1 ring-gray-200"
            :style="item.color ? { backgroundColor: item.color } : { backgroundColor: '#e5e7eb' }"
          />
          <div class="flex-1 min-w-0">
            <p class="truncate text-sm font-medium text-gray-900">{{ item.name }}</p>
            <p class="text-xs text-gray-400">{{ $t('sale_states.sort_order') }}: {{ item.sort_order }}</p>
          </div>
          <span
            v-if="item.is_default"
            class="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200"
          >
            {{ $t('sale_states.is_default') }}
          </span>
          <span
            v-if="item.is_final_state"
            class="inline-flex items-center rounded-full bg-rose-50 px-2 py-0.5 text-xs font-medium text-rose-700 ring-1 ring-rose-200"
          >
            {{ $t('sale_states.is_final_state') }}
          </span>
