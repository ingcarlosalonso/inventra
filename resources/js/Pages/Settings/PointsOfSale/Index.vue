<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('points_of_sale.title') }}</h1>
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <div v-if="loadingList" class="divide-y divide-gray-100">
        <div v-for="i in 5" :key="i" class="flex items-center gap-4 px-5 py-4 animate-pulse">
          <div class="h-6 w-10 rounded bg-gray-200" />
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-sm font-bold text-indigo-700">{{ item.number }}</div>
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ item.address || '—' }}</p>
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('points_of_sale.edit') : $t('points_of_sale.create')">
    <form class="space-y-5" @submit.prevent="save">
      <InputField
        v-model.number="form.number"
        type="number"
        :label="$t('points_of_sale.number')"
        :error="formErrors.number?.[0]"
        required
      />
      <InputField
        v-model="form.name"
        :label="$t('points_of_sale.name')"
        :error="formErrors.name?.[0]"
        required
      />
      <InputField
        v-model="form.address"
        :label="$t('points_of_sale.address')"
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('points_of_sale.delete_confirm', { name: deleteTarget?.name })"
    @confirm="doDelete"
  />
</template>
...
async function save() {
    ...
    const result = editing.value
        ? await putForm(`/api/v1/sales/points-of-sale/${editing.value.id}`, form.value)
...
async function doDelete() {
    ...
    await del(`/api/v1/sales/points-of-sale/${deleteTarget.value.id}`)
    await fetchItems()
}
