<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('order_states.title') }}</h1>
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-1.5 shrink-0">
                  <span v-if="item.is_default" class="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200">{{ $t('order_states.is_default') }}</span>
                  <span v-if="item.is_final_state" class="inline-flex items-center rounded-full bg-rose-50 px-2 py-0.5 text-xs font-medium text-rose-700 ring-1 ring-rose-200">{{ $t('order_states.is_final_state') }}</span>
                </div>
              </div>
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ $t('order_states.sort_order') }}: {{ item.sort_order }}</p>
                <div class="flex items-center gap-1.5 shrink-0">
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('order_states.edit') : $t('order_states.create')">
    <form class="space-y-5" @submit.prevent="save">
      <InputField
        v-model="form.name"
        :label="$t('order_states.name')"
        :error="formErrors.name?.[0]"
        required
      />
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-1.5 block text-sm font-medium text-gray-700">{{ $t('order_states.color') }}</label>
          <input
            v-model="form.color"
            type="color"
        ...
        <InputField
          ...
          type="number"
          :label="$t('order_states.sort_order')"
          :error="formErrors.sort_order?.[0]"
        />
      </div>
      <ToggleSwitch v-model="form.is_default" :label="$t('order_states.is_default')" />
      <ToggleSwitch v-model="form.is_final_state" :label="$t('order_states.is_final_state')" />
  ...
  <ConfirmModal
    v-model="confirmOpen"
    :title="$t('order_states.delete_confirm')"
    @confirm="doDelete"
  />
</template>
