<template>
  <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-gray-900">{{ $t('suppliers.title') }}</h1>
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="mt-0.5 flex items-center justify-between gap-2">
                  <p class="truncate text-xs text-gray-400 min-w-0">{{ [item.contact_name, item.phone, item.email].filter(Boolean).join(' · ') || '—' }}</p>
    ...
    <SlideOver v-model="slideOverOpen" :title="editing ? $t('suppliers.edit') : $t('suppliers.create')">
      <form class="space-y-5" @submit.prevent="save">
        <InputField v-model="form.name" :label="$t('suppliers.company_name')" :error="formErrors.name?.[0]" required />
        <InputField v-model="form.contact_name" :label="$t('suppliers.contact_name')" :error="formErrors.contact_name?.[0]" />
        <div class="grid grid-cols-2 gap-4">
          <InputField v-model="form.phone" :label="$t('suppliers.phone')" type="tel" :error="formErrors.phone?.[0]" />
          <InputField v-model="form.email" :label="$t('suppliers.email')" type="email" :error="formErrors.email?.[0]" />
        </div>
        <InputField v-model="form.address" :label="$t('suppliers.address')" :error="formErrors.address?.[0]" />
        <TextareaField v-model="form.notes" :label="$t('suppliers.notes')" :rows="3" />
    ...
    <ConfirmModal v-model="confirmOpen" :title="$t('suppliers.delete_confirm', { name: deleteTarget?.name })" @confirm="doDelete" />
</template>
...
function openEdit(item) {
    editing.value = item
    form.value = { name: item.name, contact_name: item.contact_name ?? '', email: item.email ?? '', phone: item.phone ?? '', address: item.address ?? '', notes: item.notes ?? '', is_active: item.is_active }
