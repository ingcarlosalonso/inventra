<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ $t('composite_products.title') }}</h1>
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-violet-100 text-sm font-semibold text-violet-700">{{ item.name.charAt(0).toUpperCase() }}</div>
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ [item.code, `${item.items?.length ?? 0} ${$t('composite_products.items')}`].filter(Boolean).join(' · ') }}</p>
  ...
  <SlideOver v-model="slideOverOpen" :title="editing ? $t('composite_products.edit') : $t('composite_products.create')">
    <form class="space-y-5" @submit.prevent="save">
      <InputField v-model="form.name" :label="$t('composite_products.name')" :error="formErrors.name?.[0]" required />
      <InputField v-model="form.code" :label="$t('composite_products.code')" :error="formErrors.code?.[0]" />

      <!-- Items -->
      <div>
        <div class="mb-2 flex items-center justify-between">
          <label class="text-sm font-medium text-gray-700">{{ $t('composite_products.items') }}</label>
          ...
          >
            <div class="flex-1">
              <SelectField
                ...
                :label="$t('composite_products.product')"
            ...
            <div class="w-24">
              <InputField
                ...
                :label="$t('composite_products.quantity')"
                type="number"
                min="1"
        ...
        <p v-if="!form.items.length" class="mt-1 text-sm text-gray-500">{{ $t('composite_products.no_items') }}</p>
  ...
  <ConfirmModal v-model="confirmOpen" :title="$t('composite_products.delete_confirm', { name: deleteTarget?.name })" @confirm="doDelete" />
</template>
...
async function doDelete() {
  ...
  const { error } = await del(`/api/v1/products/composite/${deleteTarget.value.id}`)
