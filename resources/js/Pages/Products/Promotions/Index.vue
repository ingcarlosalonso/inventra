      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group px-4 py-3 hover:bg-gray-50 transition">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-amber-100 text-sm font-semibold text-amber-700">{{ item.name.charAt(0).toUpperCase() }}</div>
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-0.5 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ [item.code, item.sale_price ? `$${item.sale_price}` : null, `${item.items?.length ?? 0} ${$t('promotions.items')}`].filter(Boolean).join(' · ') }}</p>
...
async function fetchItems(url = null) {
  ...
  const { data } = await get(url ?? '/api/v1/products/promotions', url ? {} : params)
...
async function save() {
  ...
  const result = editing.value
    ? await putForm(`/api/v1/products/promotions/${editing.value.id}`, form.value)
    : await postForm('/api/v1/products/promotions', form.value)
...
async function doDelete() { confirmOpen.value = false; await del(`/api/v1/products/promotions/${deleteTarget.value.id}`); await fetchItems() }
