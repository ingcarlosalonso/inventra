  ...
  <div class="max-w-6xl mx-auto space-y-6">
              ...
              <td class="px-3 py-2">
                <div class="relative">
                  ...
                  <input
                    ...
                    :class="formErrors[`items.${index}.unit_price`] ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500'"
                  />
                </div>
              </td>
              <td class="px-3 py-2">
      ...
      <div v-if="form.items.length > 0" class="border-t border-gray-200 px-5 py-4 bg-gray-50 rounded-b-xl">
        <div class="flex items-end justify-between gap-6">
          ...
          <div class="text-right space-y-1">
            ...
            <div v-if="saleDiscountAmount > 0" class="flex items-center justify-end gap-6 text-sm text-red-600">
              <span>{{ $t('sales.discount_amount') }}</span>
              <span class="w-28 tabular-nums">-${{ formatNumber(saleDiscountAmount) }}</span>
            </div>
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
          ...
          <div :class="['flex-1', index === 0 ? 'mt-0' : '']">
            ...
            <div class="relative">
              ...
              <input
                ...
                :class="formErrors[`payments.${index}.amount`] ? 'border-red-300' : ''"
              />
            </div>
          </div>
          <button
      ...
      <div v-else class="px-5 py-8 text-center">
        <p class="text-sm text-gray-400">{{ $t('sales.no_payments') }}</p>
      </div>

      <div v-if="paymentsTotal > 0" class="flex items-center justify-between border-t border-gray-200 px-5 py-3 bg-gray-50 rounded-b-xl">
        <span class="text-sm text-gray-500">{{ $t('sales.total') }} pagado</span>
        <span class="text-lg font-bold tabular-nums" :class="paymentsTotal >= formTotal ? 'text-emerald-600' : 'text-amber-600'">${{ formatNumber(paymentsTotal) }}</span>
      </div>
    </div>
...
const formTotal = computed(() => Math.max(subtotal.value - saleDiscountAmount.value, 0))
...
async function fetchOptions() {
  ...
  // Pre-select default state
  const defaultState = saleStates.value.find(s => s.is_default)
  if (defaultState && !form.value.sale_state_id) {
    form.value.sale_state_id = defaultState.id
  }

  // Pre-select first point of sale if only one
  if (pointsOfSale.value.length === 1 && !form.value.point_of_sale_id) {
    form.value.point_of_sale_id = pointsOfSale.value[0].id
  }

  // Add a default payment row
  if (form.value.payments.length === 0 && paymentMethods.value.length > 0) {
    addPayment()
  }
}
