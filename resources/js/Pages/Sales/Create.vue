                  ...
                  >
                    ...
                    <div class="ml-4 text-right shrink-0">
                      ...
                      <p class="text-xs font-semibold" :class="opt.stock <= 0 ? 'text-red-600' : 'text-gray-400 font-normal'">
                        {{ opt.stock <= 0 ? $t('sales.out_of_stock') : `${$t('sales.stock')}: ${opt.stock}` }}
                      </p>
                    </div>
                  </li>
                </ul>
                ...
                <tr v-for="(item, index) in form.items" :key="item._key" class="group hover:bg-gray-50 transition">
                  ...
                  <td class="px-3 py-2">
                    ...
                    <p v-if="isOverStock(item)" class="mt-1 text-xs text-red-600">
                      {{ $t('sales.insufficient_stock', { product: item.productName, requested: item.quantity, available: item.stock }) }}
                    </p>
                  </td>
                  <td class="px-3 py-2">
                    <div class="relative">
          ...
          <div v-if="form.payments.length > 0 && paymentsTotal > 0" class="border-t border-gray-200 px-5 py-4 bg-gray-50 rounded-b-xl">
            <div class="flex items-center justify-end gap-6">
              <div class="text-right">
                <p class="text-xs text-gray-400 mb-0.5">{{ $t('sales.total') }}</p>
                <p class="text-base font-semibold text-gray-700 tabular-nums">${{ formatNumber(formTotal) }}</p>
              </div>
              <div class="text-right">
                <p class="text-xs text-gray-400 mb-0.5">{{ $t('sales.payment_paid') }}</p>
              ...
              <div v-if="paymentDiff < 0" class="flex items-center gap-2 rounded-lg bg-amber-50 px-3 py-2 ring-1 ring-amber-200">
                ...
                <div>
                  <p class="text-xs text-amber-600">{{ $t('sales.payment_remaining') }}</p>
              ...
              <div v-else-if="paymentDiff === 0" class="flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 ring-1 ring-emerald-200">
                ...
                <p class="text-sm font-bold text-emerald-700">{{ $t('sales.payment_exact') }}</p>
              </div>
              <div v-else class="flex items-center gap-2 rounded-lg bg-blue-50 px-3 py-2 ring-1 ring-blue-200">
                ...
                <div>
                  <p class="text-xs text-blue-600">{{ $t('sales.payment_change') }}</p>
    ...
    <div class="sticky bottom-0 mt-6 rounded-xl bg-white shadow-lg ring-1 ring-gray-200 px-5 py-3.5 flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <button
          v-else
          type="button"
          :disabled="saving || form.items.length === 0 || form.payments.length === 0 || hasStockIssues"
...
async function save() {
  ...
  const result = await postForm('/api/v1/sales', payload)
  if (result.error) {
...
async function fetchOptions() {
  const [clientsRes, posRes, statesRes, pmRes, productsRes] = await Promise.all([
    get('/api/v1/clients'), get('/api/v1/sales/points-of-sale'), get('/api/v1/sales/states'),
