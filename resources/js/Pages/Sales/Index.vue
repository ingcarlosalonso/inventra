    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li
          v-for="item in items"
          :key="item.id"
          class="group cursor-pointer px-4 py-3.5 hover:bg-gray-50 transition"
          @click="() => router.visit(`/sales/${item.id}`)"
        >
          <div class="flex items-start gap-3">
            <!-- Icon -->
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-700 mt-0.5">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-start justify-between gap-2">
                <div class="flex flex-wrap items-center gap-1.5 min-w-0">
                  ...
                  <span
                    v-if="item.sale_state"
                    class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset"
                    :style="stateStyle(item.sale_state)"
                  >{{ item.sale_state.name }}</span>
                </div>
                <span class="shrink-0 text-sm font-semibold text-gray-900 tabular-nums">${{ formatNumber(item.total) }}</span>
              </div>

              <!-- Row 2: secondary info / payment badge + actions -->
              <div class="mt-1 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">
                  {{ [item.point_of_sale?.name, item.user?.name, formatDate(item.created_at)].filter(Boolean).join(' · ') }}
                </p>
...
async function fetchItems(url = null) {
  ...
  const { data } = await get(url ?? '/api/v1/sales', url ? {} : params)
...
async function doDelete() {
  ...
  await del(`/api/v1/sales/${deleteTarget.value.id}`)
  await fetchItems()
}
