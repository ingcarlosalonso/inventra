  ...
  <div class="space-y-6">
    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <ul v-else class="divide-y divide-gray-100">
        <li v-for="item in items" :key="item.id" class="group cursor-pointer px-4 py-3.5 hover:bg-gray-50 transition" @click="() => router.visit(`/receptions/${item.id}`)">
          <div class="flex items-start gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 mt-0.5">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2">
                <div class="flex flex-wrap items-center gap-1.5 min-w-0">
                  <p class="text-sm font-medium text-gray-900 truncate">{{ item.supplier?.name ?? $t('common.no_supplier') }}</p>
                  <span v-if="item.supplier_invoice" class="inline-flex items-center rounded-md bg-gray-100 px-2 py-0.5 text-xs text-gray-600">{{ item.supplier_invoice }}</span>
                </div>
                <span class="shrink-0 text-sm font-semibold text-gray-900 tabular-nums">${{ formatNumber(item.total) }}</span>
              </div>
              <div class="mt-1 flex items-center justify-between gap-2">
                <p class="truncate text-xs text-gray-400 min-w-0">{{ [formatDate(item.received_at), `${item.items?.length ?? 0} ${$t('receptions.items')}`].filter(Boolean).join(' · ') }}</p>
                <div class="flex items-center gap-0.5 shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition" @click.stop>
                  <Link :href="`/receptions/${item.id}`" class="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-indigo-600" :title="$t('common.view')"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></Link>
...
async function doDelete() {
  ...
  await del(`/api/v1/receptions/${deleteTarget.value.id}`)
  await fetchItems()
}
