...
class PointOfSaleControllerTest extends TenantFeatureTestCase
{
    public function test_index_returns_list(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/sales/points-of-sale')
            ->assertOk()
    ...
    {
        $this->getJson('/api/v1/sales/points-of-sale')->assertUnauthorized();
    }
    ...
    public function test_index_filters_by_search(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/sales/points-of-sale?search=Norte')
            ->assertOk()
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/sales/points-of-sale', [
                'number' => 42,
    ...
    public function test_store_validates_unique_number(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/sales/points-of-sale', ['number' => 99, 'name' => 'Duplicado Test'])
            ->assertUnprocessable()
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->putJson('/api/v1/sales/points-of-sale/non-existent-uuid', ['number' => 1, 'name' => 'X'])
            ->assertNotFound();
    }
