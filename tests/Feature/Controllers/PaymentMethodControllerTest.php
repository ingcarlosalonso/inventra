...
class PaymentMethodControllerTest extends TenantFeatureTestCase
{
    public function test_index_returns_list(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/sales/payment-methods')
            ->assertOk()
    ...
    {
        $this->getJson('/api/v1/sales/payment-methods')->assertUnauthorized();
    }
    ...
    public function test_index_filters_by_search(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/sales/payment-methods?search=Efectivo')
            ->assertOk()
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/sales/payment-methods', [
    ...
    {
        $this->postJson('/api/v1/sales/payment-methods', ['name' => 'Test'])
            ->assertUnauthorized();
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->putJson('/api/v1/sales/payment-methods/non-existent-uuid', ['name' => 'X'])
            ->assertNotFound();
    }
