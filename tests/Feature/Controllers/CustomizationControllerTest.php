...
class CustomizationControllerTest extends TenantFeatureTestCase
{
    public function test_show_returns_customization(): void
    {
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/settings/customization')
            ->assertOk()
    ...
    {
        $this->getJson('/api/v1/settings/customization')->assertUnauthorized();
    }

    public function test_update_saves_colors_and_font(): void
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/settings/customization', [
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/settings/customization', ['primary_color' => 'notacolor'])
            ->assertUnprocessable()
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/settings/customization', ['font_family' => 'Comic Sans'])
            ->assertUnprocessable()
    ...
    {
        $this->postJson('/api/v1/settings/customization', [])->assertUnauthorized();
    }
}
