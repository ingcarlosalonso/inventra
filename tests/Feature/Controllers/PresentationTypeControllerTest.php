...
class PresentationTypeControllerTest extends TenantFeatureTestCase
{
    ...
    public function test_index_returns_list(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/products/presentation-types')
            ->assertOk()
    ...
    public function test_index_filters_by_search(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/products/presentation-types?search=__TIPO_VOLUMEN__')
            ->assertOk()
    ...
    {
        $this->getJson('/api/v1/products/presentation-types')->assertUnauthorized();
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/products/presentation-types', ['name' => '__STORE_TIPO__', 'abbreviation' => '__st__', 'is_active' => true])
            ->assertCreated()
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/products/presentation-types', [])
            ->assertUnprocessable()
    ...
    public function test_store_validates_unique_name(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/products/presentation-types', ['name' => '__UNIQUE_NAME__', 'abbreviation' => '__un2__'])
            ->assertUnprocessable()
