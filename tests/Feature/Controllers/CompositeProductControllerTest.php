...
class CompositeProductControllerTest extends TenantFeatureTestCase
{
    ...
    public function test_index_returns_paginated_list(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/products/composite')
            ->assertOk()
    ...
    public function test_index_filters_by_name(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/products/composite?search=UNIQUE_KITVERANO_XYZ')
            ->assertOk()
    ...
    public function test_index_filters_by_code(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/products/composite?search=UNIQUEKIT-001-ABC')
            ->assertOk()
            ->assertJsonCount(1, 'data');
    }
    ...
    {
        $this->getJson('/api/v1/products/composite')->assertUnauthorized();
    }
    ...
    public function test_store_creates_composite_product(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/products/composite', [
                'name' => 'Kit Test',
                'items' => [
    ...
    {
        $this->postJson('/api/v1/products/composite', [])->assertUnauthorized();
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->putJson('/api/v1/products/composite/non-existent-uuid', [])
            ->assertNotFound();
    }
