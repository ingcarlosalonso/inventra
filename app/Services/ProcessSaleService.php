...
use App\Models\SaleState\Scopes\IsDefault as SaleStateIsDefault;
...
class ProcessSaleService
{
    ...
    public function execute(array $data, int $userId): Sale
    {
        return DB::connection('tenant')->transaction(function () use ($data, $userId): Sale {
            ...
            $pointOfSaleId = PointOfSale::query()->withScopes(new ByUuid($data['point_of_sale_id']))->value('id');

            $saleStateId = isset($data['sale_state_id'])
                ? SaleState::query()->withScopes(new ByUuid($data['sale_state_id']))->value('id')
                : SaleState::query()->withScopes(new SaleStateIsDefault)->value('id');
